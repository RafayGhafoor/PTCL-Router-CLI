...
class Router:
    ...
    def __init__(self, gateway="192.168.1.1", username="admin", password="ptcl"):
        if utils.validate_gateway(gateway):
            ...
            raise ValueError(
                "Invalid Gateway format specified, Valid format is", "192.168.1.1"
            )
        ...
        for i in td:
            ...
            if utils.mac_pattern.search(i.text):
                ...
                self.dev_info[hostname] = [
                    i.text,
                    td[td.index(i) + 1].text,
                    td[td.index(i) + 2].text,
                ]
    ...
    def time_limit(
        self, username="User_1", mac="", days="Everyday", start="1", end="24"
    ):
        """
        ...
        if days and len(days) < 3:
            if len(days) == 1:
                self.session.get(
                    self.gateway + "todmngr.tod?action=add&mac={}".format(mac),
                    params=gen_params(days=week_days[days[0]]),
                )
                ...
                if days[0] == days[1]:
                    self.session.get(
                        self.gateway + "todmngr.tod?action=add&mac={}".format(mac),
                        params=gen_params(days=week_days["Everyday"]),
                    )

                elif days[0] != days[1]:
                    self.session.get(
                        self.gateway + "todmngr.tod?action=add&mac={}".format(mac),
                        params=gen_params(days=str(week_days[days[1]])),
                    )
            else:
    ...
    def mac_filter(self, *mac, type="add", page="wlmacflt.cmd"):
        ...
        if type == "add":
            ...
            params = "?action={}&wlFltMacAddr={}&sessionKey={}".format(
                type, ",".join(mac), self.get_token()
            )

        else:
            params = "?action={}&rmLst={}&sessionKey={}".format(
                type, ",".join(mac), self.get_token()
            )
